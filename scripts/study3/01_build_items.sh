...
: "${STUDY3_NATURAL_ANCHORS_PER_GRANULARITY:=500}"
: "${STUDY3_CONTROLLED_ANCHORS_PER_GRANULARITY:=100}"
: "${STUDY3_CONTROLLED_K:=2,3,4,5}"
: "${STUDY3_SAMPLE_SEED:=42}"

cd "$MGHDA_ROOT"
mkdir -p "$MGHDA_DATA_ROOT/processed/study3/items"
...
python -m ghm.study3.build_items \
  ...
  --g1-output "$MGHDA_DATA_ROOT/processed/study3/items/study3_g1_multiselect_items.jsonl" \
  --g2-output "$MGHDA_DATA_ROOT/processed/study3/items/study3_g2_multiselect_items.jsonl" \
  --summary "$MGHDA_DATA_ROOT/outputs/study3/audits/study3_item_summary.json" \
